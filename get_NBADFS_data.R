get_NBADFS_data <- function() {
  ...
  maindf <- read_csv("data/nbadkdataset.csv", col_types = cols(min = col_character()))
  #fix for bad data
  # maindf <- filter(maindf, date.y <= as.Date("2-03-2017", "%m-%d-%Y"), !is.na(opp_dfsnm)) 
  maindf <- filter(maindf,!is.na(opp_dfsnm))
  mostrecentupdate <- max(maindf$date.y)
  
  
  if (mostrecentupdate < today()-1){
    print("Updating DFS data.")
    write_csv(maindf, "data/nbadkdatabackup.csv")
    maindf <- update_NBADFS(maindf)
    write_csv(maindf, "data/nbadkdataset.csv")
    maindf %>% filter(Price > 0 ,Value > 0) %>% write_csv("data/nbadk.csv")
    todaydf <- get_Today_SummaryTab(maindf) 
    todaydf %>% write_csv(.,"data/dktab.csv")
    print(paste0("Data Updated as of: ",max(maindf$date.y)))
  }
...
get_daily_DFS <- function(date){
  ...
  if(date <= as.Date("11-14-2016", "%m-%d-%Y") && ("Lance Stephenson" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Lance Stephenson",]$Team <- "NOR"
  }
  
  if(date >= as.Date("2-08-2017", "%m-%d-%Y") && ("Lance Stephenson" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Lance Stephenson",]$Team <- "MIN"
  }
  ...
  if(date <= as.Date("1-28-2016", "%m-%d-%Y") && ("Yogi Ferrell" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Yogi Ferrell",]$Team <- "BKN"
  }
  if(date >= as.Date("1-29-2016", "%m-%d-%Y") && ("Yogi Ferrell" %in% dailysumm$Name)){
      dailysumm[dailysumm$Name == "Yogi Ferrell",]$Team <- "DAL"
  }
  if(date >= as.Date("1-29-2016", "%m-%d-%Y") && ("Yogi Ferrell" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Yogi Ferrell",]$Team <- "DAL"
  }
  if(date <= as.Date("2-11-2017", "%m-%d-%Y") && ("Jusuf Nurkic" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Jusuf Nurkic",]$Team <- "DEN"
  }
  
  if(date <= as.Date("2-14-2017", "%m-%d-%Y") && ("Terrence Ross" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Terrence Ross",]$Team <- "TOR"
  }
  
  if(date <= as.Date("2-14-2017", "%m-%d-%Y") && ("Serge Ibaka" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Serge Ibaka",]$Team <- "ORL"
  }
  
  if(date <= as.Date("2-9-2017", "%m-%d-%Y") && ("Mason Plumlee" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Mason Plumlee",]$Team <- "POR"
  }
  
  if(date <= as.Date("2-2-2017", "%m-%d-%Y") && ("Roy Hibbert" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Roy Hibbert",]$Team <- "CHA"
  }
  
  if(date <= as.Date("2-2-2017", "%m-%d-%Y") && ("Spencer Hawes" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Spencer Hawes",]$Team <- "CHA"
  }
  
  if(date <= as.Date("2-1-2017", "%m-%d-%Y") && ("Miles Plumlee" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Miles Plumlee",]$Team <- "MIL"
  }
  
  if(date <= as.Date("2-16-2017", "%m-%d-%Y") && ("Taj Gibson" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Taj Gibson",]$Team <- "CHI"
  }
  
  if(date <= as.Date("2-16-2017", "%m-%d-%Y") && ("Doug McDermott" %in% dailysumm$Name)){
    dailysumm[dailysumm$Name == "Doug McDermott",]$Team <- "CHI"
  }
...
get_NBADFS_btwn <- function(start_date, end_date) {
  ...
  bad_dates <- c("11-24-2016", "12-24-2016", "02-17-2017", "02-18-2017", "02-19-2017", "02-20-2017", "02-21-2017", "02-22-2017")
...
#testdf %>% filter(Pos %in% c("SF", "SF/PF", "SG/SF"), !is.na(mins), start == "Y") %>% mutate(usage = as.numeric(USG)) %>% select(Value, pts, X3, reb, ast, stl, usage, mins, totals) %>% corrgram(order=TRUE, lower.panel=panel.shade, upper.panel=panel.pie, text.panel=panel.txt)

dk_Salary_File <- function() {
  library(dplyr)
  path = './data/'
  file.names <- dir(path, pattern ="DKSalaries.csv")
  date <- as.character(today()-1)
  date <- gsub('\\.csv', date,file.names)
  print(date)
...
get_Today_DK <- function(){
  ...
  if("D.J. Augustine" %in% todaydk$Name){
    todaydk[todaydk$Name == "D.J. Augustine",]$Name <- "DJ Augustine"
  }
  if("P.J. Tucker" %in% todaydk$Name){
    todaydk[todaydk$Name == "P.J. Tucker",]$Name <- "PJ Tucker"
  }
  if("C.J. McCollum" %in% todaydk$Name){
    todaydk[todaydk$Name == "C.J. McCollum",]$Name <- "CJ McCollum"
  }
  if("C.J. Miles" %in% todaydk$Name){
    todaydk[todaydk$Name == "C.J. Miles",]$Name <- "CJ Miles"
  }
  if("C.J. Watson" %in% todaydk$Name){
    todaydk[todaydk$Name == "C.J. Watson",]$Name <- "CJ Watson"
  }
  if("C.J. Wilcox" %in% todaydk$Name){
    todaydk[todaydk$Name == "C.J. Wilcox",]$Name <- "CJ Wilcox"
  }
  if("A.J. Hammons" %in% todaydk$Name){
    todaydk[todaydk$Name == "A.J. Hammons",]$Name <- "AJ Hammons"
  }
  if("Derrick Jones Jr." %in% todaydk$Name){
    todaydk[todaydk$Name == "Derrick Jones Jr.",]$Name <- "Derrick Jones"
  }
  if("Kelly Oubre Jr." %in% todaydk$Name){
    todaydk[todaydk$Name == "Kelly Oubre Jr.",]$Name <- "Kelly Oubre"
  }
  if("Larry Nance Jr." %in% todaydk$Name){
    todaydk[todaydk$Name == "Larry Nance Jr.",]$Name <- "Larry Nance"
  }
  if("Tim Hardaway Jr." %in% todaydk$Name){
    todaydk[todaydk$Name == "Tim Hardaway Jr.",]$Name <- "Tim Hardaway"
  }
  if("Stephen Zimmerman Jr." %in% todaydk$Name){
    todaydk[todaydk$Name == "Stephen Zimmerman Jr.",]$Name <- "Stephen Zimmerman"
  }
  if("J.J. Redick" %in% todaydk$Name){
    todaydk[todaydk$Name == "J.J. Redick",]$Name <- "JJ Redick"
  }
  
  if("J.J. Barea" %in% todaydk$Name){
    todaydk[todaydk$Name == "J.J. Barea",]$Name <- "JJ Barea"
  }
  if("T.J. Warren" %in% todaydk$Name){
    todaydk[todaydk$Name == "T.J. Warren",]$Name <- "TJ Warren"
  }
  if("T.J. McConnell" %in% todaydk$Name){
    todaydk[todaydk$Name == "T.J. McConnell",]$Name <- "TJ McConnell"
  }
...
# testdf %>% filter(Opp %in% c('PHO', 'WAS', 'LAL', 'MEM' , 'DET'), Price > 6000, Pos %in% c('SF', 'SG/SF')) %>% ggplot(aes(x = Value,fill = Opp, colour = Opp)) +
